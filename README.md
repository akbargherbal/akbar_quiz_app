...
Both scripts need to be run from the `src/` directory of the project, with your Django virtual environment activated.

### 1. `dir_import_chapter_quizzes.py` (Bulk Import from Directory)

This script scans a specified directory (by default, `QUIZ_COLLECTIONS/` in the project root) for `.pkl` files and imports the quiz data from each.
...
This command will:
- Look for `.pkl` files in the `PROJECT_ROOT/QUIZ_COLLECTIONS/` directory.
- Process each file, creating quizzes and questions based on the data within.
- Log its progress and a summary to the console and to a timestamped log file in the `logs/` directory.
...
*   `--simple-titles`: (Optional) Uses a simpler format for quiz titles (e.g., "Chapter 1 - Quiz 1") instead of the more descriptive default (e.g., "Chapter 1: Topic A - Quiz 1").
...
```
(Note: `--test-file` here simply provides the file path directly, it doesn't imply a "test mode" in the same way as `dir_import_chapter_quizzes.py` unless `--test` is also specified for generated data.)
...
*   `--system-category "Category Name"`: (Optional) Assigns the imported quiz to the specified `SystemCategory`.
...
*   **Dependencies:** Make sure all project dependencies, including `pandas`, are installed from `requirements.txt`.
...
*   **Idempotency:** If a quiz with the exact same generated title already exists in the database, the scripts will typically log a warning and skip re-creating that specific quiz to avoid duplicates. Questions are generally tied to the quiz they are imported with.
