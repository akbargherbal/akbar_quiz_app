...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "file_path",
            type=str,
            help="Path to the quiz bank file (CSV or Excel)"
        )
        parser.add_argument(
            "--quiz-title",
            type=str,
        ...
        parser.add_argument(
            "--topic",
            type=str,
            help="Topic to associate with the imported questions"
        )
        parser.add_argument(
            "--max-questions",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if file_ext == '.csv':
                ...
                self.stderr.write(self.style.ERROR(f"Unsupported file format: {file_ext}"))
                return
                
            self.stdout.write(self.style.SUCCESS(f"Loaded {len(df)} rows from {file_path}"))
            ...
            if chapter_column in df.columns and 'chapter_no' not in df.columns:
    ...
    def _import_as_single_quiz(self, df, quiz_title, topic_name, max_questions):
        ...
        try:
            if max_questions and max_questions < len(df):
                self.stdout.write(self.style.NOTICE(f"Sampling {max_questions} questions from {len(df)} total"))
    ...
    def _import_by_topic(self, df, topic_column, max_questions):
        ...
        if topic_column not in df.columns:
            self.stderr.write(self.style.ERROR(f"Topic column '{topic_column}' not found in data"))
            return
        ...
        for topic in topics:
            ...
            if len(topic_df) == 0:
                self.stdout.write(self.style.WARNING(f"No questions found for topic: {topic}"))
                continue
            ...
            try:
                ...
                self.stderr.write(self.style.ERROR(f"Error creating quiz for topic '{topic}': {str(e)}"))
