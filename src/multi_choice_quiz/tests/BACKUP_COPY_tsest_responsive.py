...
# --- Setup Logging ---
log_format = '%(asctime)s - %(levelname)s - %(name)s - [%(filename)s:%(lineno)d] - %(message)s' # Added filename/lineno
...
def check_results_panel_visibility(page: Page, breakpoint_name: str):
    ...
    # --- MODIFIED LOCATOR (using ID) ---
    results_panel_locator_str = '#quiz-results-panel' # Use the newly added ID
    ...
    expect(results_panel, f"Results panel container should be visible at {breakpoint_name}").to_be_visible(timeout=POST_EVENT_UI_TIMEOUT + 3000) # Extra time after completion event
    logger.info("Results panel container is visible.") # Added confirmation log

    results_heading = results_panel.locator('h3:text("Quiz Results")')
    expect(results_heading, f"Results heading should be visible at {breakpoint_name}").to_be_visible()

    # Note: The original ':has-text("Rating")' might be okay, but let's make the stats selector slightly more robust relative to the panel
    stats_section = results_panel.locator('div:has-text("Rating")').first # Ensure we get the first one if structure is nested
    # stats_section = results_panel.locator('div.md\\:flex > div:has-text("Rating")') # Keep original if preferred
    expect(stats_section, f"Stats section should be visible at {breakpoint_name}").to_be_visible()
    expect(stats_section.locator('span:text("Score")'), "Score label should be visible").to_be_visible()
    expect(stats_section.locator('span:text("Percentage")'), "Percentage label should be visible").to_be_visible()
    expect(stats_section.locator('span:text("Time")'), "Time label should be visible").to_be_visible()
    expect(stats_section.locator('span:text("Rating")'), "Rating label should be visible").to_be_visible()

    mistakes_section = results_panel.locator('div:has-text("Mistakes Review")').first
    expect(mistakes_section, f"Mistakes section should be visible at {breakpoint_name}").to_be_visible()
    mistakes_list = mistakes_section.locator("ul")
    expect(mistakes_list, f"Mistakes list should be visible at {breakpoint_name}").to_be_visible()

    go_home_button = results_panel.locator('a[href="/"]:text("Go Home")')
    expect(go_home_button, f"Go Home button should be visible at {breakpoint_name}").to_be_visible()
    expect(go_home_button, f"Go Home button should be enabled at {breakpoint_name}").to_be_enabled()

    play_again_button = results_panel.locator('button:text("Play Again")')
    expect(play_again_button, f"Play Again button should be visible at {breakpoint_name}").to_be_visible()
    expect(play_again_button, f"Play Again button should be enabled at {breakpoint_name}").to_be_enabled()
...
def test_results_layout_responsiveness(page: Page, name: str, size: dict):
    ...
    try:
        ...
        expect(quiz_container, "Quiz container should become visible").to_be_visible(timeout=DEFAULT_VISIBILITY_TIMEOUT)
        expect(quiz_container, "Quiz container should have x-data").to_have_attribute("x-data", "quizApp()", timeout=5000)
        ...
        expect(first_option, "First option button should render after init").to_be_visible(timeout=DEFAULT_VISIBILITY_TIMEOUT)
        expect(first_option, "First option button should have text content").not_to_be_empty(timeout=5000)
        ...
        try:
            ...
            try:
                 ...
                 expect(quiz_data_script, "Quiz data script should be attached").to_be_attached(timeout=5000)
        ...
        for i in range(question_count):
            ...
            expect(current_options.first, f"Options for Q{q_num} should be visible before click").to_be_visible(timeout=POST_EVENT_UI_TIMEOUT)
            option_to_click = current_options.first
            expect(option_to_click, f"Option 1 for Q{q_num} should be enabled before click").to_be_enabled(timeout=POST_EVENT_UI_TIMEOUT)
            ...
            page.evaluate(f"""() => {{
                ...
                window['{listener_function_name}'] = (event) => {{
                    ...
                    // Optional: Remove listener after first catch if needed
            ...
            if expected_event_name == "quiz:question-changed":
                ...
                try:
                    ...
                    expect(next_options.first, f"First option for Q{next_q_num} should appear after event").to_be_visible(timeout=POST_EVENT_UI_TIMEOUT)
                    expect(next_options.first, f"First option for Q{next_q_num} should be enabled after event").to_be_enabled(timeout=POST_EVENT_UI_TIMEOUT)
                    # Optional: Check if the text of the first option changed to be extra sure
        ...
        # results_panel is already defined inside check_results_panel_visibility,
        # but we need it here too. Use the same robust ID locator.
        ...
        if play_again_box and go_home_box:
            ...
            if size["width"] < 640:
                 ...
                 # Expect vertical separation > horizontal
                 expect(y_diff, f"Y diff ({y_diff:.1f}) should be > ~0.5 button height ({button_height * 0.5:.1f}) (stacked) at {name}").to_be_greater_than(button_height * 0.5)
                 expect(x_diff, f"X diff ({x_diff:.1f}) should be relatively small (stacked) at {name}").to_be_less_than(button_width * 0.5) # Allow some horizontal diff
                 logger.info(f"Button stacking verified for {name}.")
            else:
                 logger.info(f"Verifying button row layout for {name} (>= 640px)")
                 # Expect horizontal separation > vertical
                 expect(y_diff, f"Y diff ({y_diff:.1f}) should be small (< 15px) (row) at {name}").to_be_less_than(15) # Allow tolerance for alignment
                 expect(x_diff, f"X diff ({x_diff:.1f}) should be > ~0.5 button width ({button_width * 0.5:.1f}) (row) at {name}").to_be_greater_than(button_width * 0.5)
