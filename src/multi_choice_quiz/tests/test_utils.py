...
class TestDataframeImport(TestCase):
    ...
    def setUp(self):
        ...
        self.standard_df = pd.DataFrame({
            'text': ['What is the capital of France?', 'Which planet is known as the Red Planet?'],
            'options': [
        ...
        self.alt_columns_df = pd.DataFrame({
            'question_text': ['What is the formula for water?', 'What is the chemical symbol for gold?'],
            'options': [
    ...
    def test_json_string_options(self):
        ...
        option_texts = list(question.options.order_by('position').values_list('text', flat=True))
        self.assertEqual(option_texts, ["Hamburg", "Munich", "Berlin", "Frankfurt", "Cologne"])
    ...
    def test_csv_string_options(self):
        ...
        # Verify options were parsed correctly
        option_texts = list(question.options.order_by('position').values_list('text', flat=True))
        self.assertEqual(option_texts, ["English", "Mandarin", "Spanish", "Hindi", "Arabic"])
    ...
    def test_sample_size_parameter(self):
        ...
        larger_df = pd.concat([self.standard_df, self.alt_columns_df], ignore_index=True)
...
class TestCurateData(TestCase):
    ...
    def setUp(self):
        ...
        self.df = pd.DataFrame({
            ...
            'options': [
                [f'Option {j}' for j in range(1, 6)] for i in range(1, 21)
            ],
    ...
    def test_column_mapping(self):
        ...
        alt_df = pd.DataFrame({
            ...
            'options': [
                [f'Option {j}' for j in range(1, 6)] for i in range(1, 6)
            ],
